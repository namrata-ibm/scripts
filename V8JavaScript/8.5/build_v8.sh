...
# Instructions:
# Download build script: wget https://raw.githubusercontent.com/linux-on-ibm-z/scripts/master/V8JavaScript/8.4/build_v8.sh
# Execute build script: bash build_v8.sh    (provide -h for help)


set -e -o pipefail
PACKAGE_NAME="v8"
PACKAGE_VERSION="8.5.210.20"
CURDIR="$(pwd)"
...
function installGCC() {
    ...
    sudo ln -sf $CURDIR/gcc/gcc-7.5.0/objdir/s390x-linux-gnu/libatomic/.libs/libatomic.so.1 /usr/lib/s390x-linux-gnu/libatomic.so
    set -e
}
...
cat << EOF > setenv.sh
        ...
        export PATH=$CURDIR/gn/out:$PATH
	      ...
	      export C_INCLUDE_PATH=/opt/gcc/lib/gcc/s390x-linux-gnu/7.5.0/include
	      export CPLUS_INCLUDE_PATH=/opt/gcc/lib/gcc/s390x-linux-gnu/7.5.0/include
...
case "$DISTRO" in
    "ubuntu-20.04")
		...
		libpango1.0-dev libgconf2-dev gnome-keyring libatk1.0-dev libgtk-3-dev wget clang g++ gcc ninja-build gcc-multilib \
